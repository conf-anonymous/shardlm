#!/usr/bin/env python3
"""
Test the BATCHED PREFILL endpoint.
Processes ALL prompt tokens through ALL 28 layers in ONE request.

Expected: Massive speedup for prompt processing (e.g., 26 tokens * 28 layers = 728 ops in 1 request)
"""
...
def softmax(scores: List[float]) -> List[float]:
    ...
    exps = [math.exp(min(s - max_score, 20)) for s in scores]
    total = sum(exps)
...
class KVCache:
    """Client-side KV cache structure matching server format."""
    ...
    def set_from_prefill(self, k_cache: List[List[List[float]]], v_cache: List[List[List[float]]]):
        """Set cache from prefill response."""
...
def batched_prefill(session_id: str, hidden_client: List[List[float]], hidden_server: List[List[float]]) -> Tuple[List[float], List[float], List[List[List[float]]], List[List[List[float]]], List[float], List[float]]:
    """
    Process ALL prompt tokens through ALL layers in ONE request.
    Returns: (final_hidden_client, final_hidden_server, k_cache, v_cache, logits_client, logits_server)
    """
    ...
    if resp.status_code != 200:
        raise ValueError(f"Batched prefill failed: {resp.status_code} {resp.text[:500]}")
...
def test_batched_prefill():
    print("=" * 70)
    print("BATCHED PREFILL + BATCHED GENERATION TEST")
    print("ALL prompt tokens + ALL layers in ONE request!")
    print("=" * 70)
    ...
    # BATCHED PREFILL: All tokens, all layers, ONE request!
    print(f"\n3. BATCHED PREFILL ({len(prompt_tokens)} tokens × 28 layers in ONE request)...")
    ...
    final_hidden_client, final_hidden_server, k_cache_data, v_cache_data, logits_client, logits_server = batched_prefill(
        session_id, hidden_client, hidden_server
    )

    # Set the KV cache from prefill response
    kv_cache.set_from_prefill(k_cache_data, v_cache_data)

    prefill_time = time.time() - prefill_start
    ops_count = len(prompt_tokens) * 28  # tokens * layers
    print(f"   Prefill complete in {prefill_time:.2f}s")
    print(f"   {ops_count} layer-token operations in ONE HTTP request!")
    print(f"   Throughput: {len(prompt_tokens)/prefill_time:.1f} prompt tokens/s")

    # Generate tokens using batched generation
    print(f"\n4. Generating response (batched generation)...")
    generated_tokens = []
    max_new_tokens = 20
    EOS_TOKEN = 151645

    gen_start = time.time()
    hidden_client_gen = final_hidden_client
    hidden_server_gen = final_hidden_server
    ...
    for step in range(max_new_tokens):
        ...
        hidden_client_gen = sanitize_list(next_embed['client_shares'][0])
        hidden_server_gen = sanitize_list(next_embed['server_shares'][0])

        position = kv_cache.seq_len

        # ALL 28 layers + logits in ONE request!
        hidden_client_gen, hidden_server_gen, logits_client, logits_server = process_token_batched(
            session_id, hidden_client_gen, hidden_server_gen, position, kv_cache
        )
    ...
    print(f"\n" + "=" * 70)
    print("BATCHED PREFILL RESULTS")
    print("=" * 70)
    print(f"User: {user_message}")
    print(f"Response: {response}")
    print(f"\nTokens generated: {len(generated_tokens)}")
    print(f"Prefill time: {prefill_time:.2f}s ({len(prompt_tokens)/prefill_time:.1f} tok/s)")
    print(f"Generation time: {gen_time:.2f}s")
    print(f"Total time: {total_time:.2f}s")
    if gen_time > 0 and len(generated_tokens) > 0:
    ...
    batched_per_token_total = 11.2  # Previous best
    print(f"CPU (112 requests/token):         {cpu_total:.1f}s")
    print(f"GPU per-layer (4 req/layer):      {gpu_per_layer_total:.1f}s")
    print(f"GPU batched (1 req/token):        {batched_per_token_total:.1f}s")
    print(f"GPU batched prefill (1 req/all):  {total_time:.1f}s")
    print(f"Speedup vs CPU:                   {cpu_total/total_time:.1f}x")
    print(f"Speedup vs GPU per-layer:         {gpu_per_layer_total/total_time:.1f}x")
    print(f"Speedup vs batched per-token:     {batched_per_token_total/total_time:.1f}x")
    print("=" * 70)
    print("BATCHED PREFILL TEST COMPLETE!")
    print("=" * 70)
