...
if [ "$MODEL_CHOICE" != "4" ]; then
    ...
    echo "  Tokenizer:     $DEFAULT_MODEL_DIR/tokenizer.json"
fi
echo ""
echo "========================================"
echo "  Quick Start"
...
echo "   SHARDLM_V2_NUM_GPUS=1 \\"
echo "   SHARDLM_V2_PORT=9090 \\"
echo "   ./target/release/shardlm-v2-server"
echo ""
echo "2. In another terminal, run generation with streaming output:"
echo ""
echo "   ./target/release/shardlm-v2-client generate \\"
echo "     -s http://localhost:9090 \\"
echo "     -p \"Hello!\" \\"
echo "     --max-tokens 50 \\"
echo "     --timing \\"
echo "     --tokenizer $DEFAULT_MODEL_DIR/tokenizer.json"
...
echo ""
echo "   ./target/release/shardlm-v2-client chat \\"
echo "     -s http://localhost:9090 \\"
echo "     --tokenizer $DEFAULT_MODEL_DIR/tokenizer.json"
echo ""
echo "4. Run benchmarks:"
echo ""
echo "   ./target/release/shardlm-v2-client benchmark \\"
echo "     -s http://localhost:9090 \\"
echo "     --runs 10 --warmup 2 --endpoint v2"
echo ""
echo "========================================"
echo "  Protocol Versions & Feature Flags"
echo "========================================"
echo ""
echo "The default build (--features cuda) includes V2, V3, and V3-OT endpoints."
echo "To enable V3-CC or V3-MPC, rebuild with additional feature flags:"
echo ""
echo "   # V3-CC (H100 Confidential Computing)"
echo "   cargo build -p shardlm-v2-server --features h100-cc,cuda --release"
echo ""
echo "   # V3-MPC (Beaver triple-based MPC)"
echo "   cargo build -p shardlm-v2-server --features mpc-secure,cuda --release"
echo ""
echo "   # All features"
echo "   cargo build -p shardlm-v2-server --features h100-cc,mpc-secure,cuda --release"
echo ""
echo "Without the appropriate feature flags, V3-CC and V3-MPC endpoints return 404."
echo ""
echo "See docs/RUNNING_EACH_VERSION.md for detailed instructions on"
