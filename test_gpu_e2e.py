#!/usr/bin/env python3
"""
GPU End-to-End test using GPU-accelerated endpoints.
Uses /v2/secure/gpu/* endpoints for matrix operations.
"""
...
def process_layer_gpu(session_id, hidden_client, hidden_server, layer_idx, position, kv_cache, model_info):
    """Process a layer using GPU-accelerated endpoints."""
    ...
    resp = requests.post(f"{SERVER_URL}/v2/secure/gpu/layer/step", json={
    ...
    if resp.status_code != 200:
        raise ValueError(f"GPU QKV failed: {resp.status_code} {resp.text[:200]}")
    qkv = resp.json()
    ...
    # Client-side attention (softmax is nonlinear, must stay on client)
    ...
    # GPU-accelerated O projection + add residual
    resp = requests.post(f"{SERVER_URL}/v2/secure/forward/batched", json={
        "session_id": session_id,
        "hidden_client": residual_input_client,
    ...
    if resp.status_code != 200:
        raise ValueError(f"O projection failed: {resp.status_code} {resp.text[:200]}")
    phase2a = resp.json()
    ...
    # Client normalizes for post_attn_layernorm
    norm_client, norm_server = client_rms_norm(hidden_after_attn_client, hidden_after_attn_server)

    # GPU-accelerated FFN gate/up projection
    resp = requests.post(f"{SERVER_URL}/v2/secure/gpu/ffn", json={
        "session_id": session_id,
        "hidden_client": norm_client,
        "hidden_server": norm_server,
        "layer_idx": layer_idx
    }, timeout=60)
    if resp.status_code != 200:
        raise ValueError(f"GPU FFN failed: {resp.status_code} {resp.text[:200]}")
    ffn_result = resp.json()

    gate_client = sanitize_list(ffn_result['gate_client'])
    gate_server = sanitize_list(ffn_result['gate_server'])
    up_client = sanitize_list(ffn_result['up_client'])
    up_server = sanitize_list(ffn_result['up_server'])

    # Client-side SiLU activation (nonlinear, must stay on client)
    activated_client, activated_server = client_swiglu(gate_client, gate_server, up_client, up_server)

    # GPU-accelerated down projection + residual
    resp = requests.post(f"{SERVER_URL}/v2/secure/gpu/ffn/down", json={
        "session_id": session_id,
    ...
    if resp.status_code != 200:
        raise ValueError(f"GPU FFN down failed: {resp.status_code} {resp.text[:200]}")
    down_result = resp.json()

    return sanitize_list(down_result['hidden_client']), sanitize_list(down_result['hidden_server'])


def test_gpu_generation():
    print("=" * 70)
    print("GPU-ACCELERATED END-TO-END GENERATION TEST")
    print("=" * 70)
    ...
    # Process all prompt tokens with GPU acceleration
    print(f"\n3. Processing prompt through {model_info['num_layers']} layers (GPU)...")
    ...
    for token_idx in range(len(prompt_tokens)):
        ...
        for layer_idx in range(model_info['num_layers']):
            hidden_client, hidden_server = process_layer_gpu(
                session_id, hidden_client, hidden_server,
                layer_idx, position, kv_cache, model_info
            )
    ...
    for step in range(max_new_tokens):
        ...
        # GPU-accelerated logits computation
        resp = requests.post(f"{SERVER_URL}/v2/secure/gpu/logits", json={
            "session_id": session_id,
